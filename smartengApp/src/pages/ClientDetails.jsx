...
function ClientDetails() {
  ...
  // Create function for handling the client state update outside the Field
        ...
        <Container className="mx-auto flex flex-col relative bg-violet-50 px-6 py-20 sm:px-10">
          ...
          <div className="grid grid-cols-4 gap-2 pt-10">
            {values.map((field, index) => (
              <div key={index} className={`overflow-auto ${field.fieldName === "Endereço" ? "col-span-2" : ""}`}>
                <Field fieldName={field.fieldName} children={field.value} onSubmit={handleChange} index={index} editable={field.fieldName === "Criado por" || field.fieldName === "Criado em" || field.fieldName === "Atualizado por" || field.fieldName === "Atualizado em" ? false : true} className="p-1"/>
              </div>
            ))}
          </div>
        </Container>
